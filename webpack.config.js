...
const pluginConfig = { ...baseConfig, entry: './src/index.ts',
	resolve: {
		...
		// HARPER: harper.js is an ESM-only package whose "exports" map exposes only the "import"
		// condition (no "require"/"default"). target:node otherwise resolves with ["node","require"]
		// and fails with '"." is not exported'. Adding "import" lets webpack pick ./dist/index.js.
		// "require" stays first so ordinary dual CJS/ESM deps still resolve to their CJS build.
...
// HARPER: the CM6 content script must consume Joplin's OWN copies of these modules — Joplin
// injects them at runtime. Marking them as commonjs externals keeps `require('@codemirror/...')`
// in the emitted bundle instead of bundling a duplicate CodeMirror (which breaks the editor).
externals: {
